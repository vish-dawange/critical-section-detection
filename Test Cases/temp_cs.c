 /********** Multithreading program for finding employee of the day **********/
  ...
  struct employee
  {
  	...
  	char department[30];
  	int room_number;
  }a,b,x;// global variable - our employees array, with 2 employees
  ...
  void* do_loop(void* data)
  {
	 ...
	 //*** Insert synchronization mechanism here (by cs_check tool) 
  ...
  int main(int argc, char* argv[])
  {
  	// loop counter
  	int i;
  	// thread ID for the first new thread
  	int thr_id1;
  	// thread ID for the second new thread
  	int thr_id2;
  	// first thread's structure
  	pthread_t p_thread1;
  	// second thread's structure
  	pthread_t p_thread2;
  
  	int num1= 1; // thread 1 employee number
  	int num2= 2; // thread 2 employee number
  
  	// local copy of 'employee of the day'.
  	struct employee eotd;
  	// pointer to currently checked employee

...
/*
   get_parameter function make the entries of parameters
   defined in user-defined funtions.
 */
...
/*
  get_log_entry function fills the log table. Lo table contains
  entries of objects used in function definitions. Function will
  make entry of object based on its type.

  If object used is declared locally then it is actually a local
  memory or if it declared globally then it is accessing global
  memory. Othrerwise we will make entry of it into temp_function_call
  structure assuming that it is function call.
 */
void get_log_entry(char log_obj[])
{
	...
	{
	    if (sym_tab[i].func_index == global_func_index - 1 && strcmp(sym_tab[i].sym_name,log_obj) == 0 && (!extern_flag)) // extern flag helps
													//to identify shadowing of global variable.
		{
		    local_found = 1;
    ...
    // If not local then check for global variable declaration
    if (!local_found)
	{
    ...
    // otherwise make entry of it into temp_function_call table assuming the function call
...
/*
  generate_tree_head function will create indivisual tree heads
  for each user-defined functions. The function calls made in those
  user-defined function definitions will be inserted at the end of
  link list.
 */
void generate_tree_head()
{
    ...
    /* loop for creating head nodes for each user-defined function
       occured in C source code. */
...
/*
  Generate_trace_tree function will look for all function calls
  made in functions defined by user.
  The node will be inserted at the end of particular list where
  head node is function in which function call is made.
 */
int generate_trace_tree()
{
	...
	{
	    // look for all functions called by other function
	    for(i = 0; i < global_func_index; i++)
		{
		    // find entry of function into call_trace_table which is being called.
		    for (j = 0; j < call_trace_index; j++)
			{
			    if (i == call_trace[j].parent_index)
				{
				    // check if that function is properly syncronized by using semaphores
				    ...
				    // check if that function is properly syncronized by using mutex
				    ...
				    // check for thread function which is being called by other function
				    ...
				    // otherwise make entry of it into trace tree
...
/*
  Before traversing trace tree initialize visited array
 */
void init_visited()
{
    int i;
...
/*
  DFS will trace the function calls made by thread functions.
  And will look for critical sections caused by function calls.
 */
void DFS(call_trace_tree *G[],int i,int visited[])
{
    ...
    // Traverse the tree using recursive DFS
    p = G[i];
    visited[i] = 1;
    while (p != NULL)
	{
	    ...
	    if (!visited[i])
		{
		    // Check if calling function causing any critical section
...
/*
  process_header function will extract all user-defined headers
  in given C source code.

  header_text contains "include "header_name.h"" and which is actually
  user-defined header.
 */
void process_header(char header_text[])
{
    ...
    /* hdr_file is substring containing double quote to extract only
       name of header. */
...
/*
  extract_archieve will extract source path of input file, like source directory
  including input file.

  source_path will help to find out headers of input file.
 */
...
/*
  get_symbol function will make the entry of variable based on
  their declaration type i.e. global or loacal.
 */
void get_symbol(char var[])
{
    if (struct_flag || thread_lib_flag)
	return;
    if(flag == 0 || (flag == 1 && extern_flag == 1)) // Global variable entry (Shadowing of global memory)
	{
	    if (!extern_flag)
		{
...
/*
  update_log_tab updates log table based on the entry if that
  belongs to any thread function.
*/
...
/*
  find_log_entries function will find entries of functions definition
  if that is called again by another thread.

  If multiple threads accessing same function then we have to make entry of
  log again into the log table without parsing that function.
*/
void find_log_entries(int func_index, int thread_id)
{
    ...
    /* The entries found for particular function definition
       has to be updated in log_table. */
    update_log_tab(copy_log_index,copy_index, thread_id);
}

/*
   check_threads will helps to update log of threading function.

   This is really useful for finding functions which are accessed
   by multiple threads. Because we have to differentiate them by
   using different thread id's.
 */
void check_threads()
{
    int i, j;
...
/*
  assign_func_index will assign index of functions from  function_table
  into thread table.

  The code is written for one pass parsing so it is difficult to find
  out the funtion which belongs to any particular thread having which
  function information before it has been written. so after parsing
  we will map the entries of those function table to thread table.
 */
...
/*
   The entries made in log table are based on function definitions.
   If that entries are belongs to any particular thread then we have
   to take care of it.

   check_thread_entry will look for such a thread functions and it
   will update log table by adding thread indexes.
 */
...
/*
  create_main_thread will create main thread for given C source.
  And it will make entry of it into thread table with new thread ID.
 */
...
/*
  cs_check function finds critical section in C multithreaded program.

  Based on the cases defined for detecting critical section such as
  synchronization, nested function calls, Global memory the function
  will generate the output.
*/
void cs_check()
{
    int i, j, k, l;
    int detect_join, synchro;
    ...
    for (i = 0; i < log_index; i++)
	{
	    ...
	    */
	    ...
	    /* Check for global memory accessed by thread function. */
	    if ( (log_tab[i].thread_func) && strcmp(log_tab[i].type,"Global") == 0)
		{

		    /* Loop for managing synchronized region if any due to semaphores */
		    for (k = 0; k < semphr_index; k++)
			if (sem_tab[k].sem_wait_point <= log_tab[i].line_number && sem_tab[k].sem_post_point >= log_tab[i].line_number)
			    break;

		    /* Loop for managing synchronized region if any due to mutexes. */
		    for (l = 0; l < mutex_index; l++)
			if (mutex_tab[l].mutex_lock_point <= log_tab[i].line_number && mutex_tab[l].mutex_unlock_point >= log_tab[i].line_number)
			    break;

		    if (k != semphr_index || l != mutex_index)
			{
			    synchro = 1;//continue;
			}


		    /* Check for other entries which are causing same global memory but different thread ID */
		    for (j= i + 1; j < log_index; j++)
			{
			    if ( (log_tab[j].thread_func) && log_tab[i].thread_index != log_tab[j].thread_index  && strcmp(log_tab[j].type,"Global") == 0 && strcmp(log_tab[i].sym_name,log_tab[j].sym_name) == 0)
				{
				    ...
				    if (!synchro)
					{
					    /* code for inserting shared object in critical section (check for duplicate )*/
				    ...
				    /* Loop for managing synchronized region if any due to mutex */
				    for (l = 0; l < mutex_index; l++)
				    ...
				    /* Before making entry into critical section table check for duplicate entries */
				    for (k = 0;k < cs_index; k++)
					{
...
void display_global_variables()
{
	...
	printf("\n\n No global variable entries found for given input.");
}
...
void display_function()
{
	...
	printf("\n\n No user-defined function entries found for given input.");

}

/* Display local_variables table */
void display_local_variables()
{
	...
	printf("\n\n No local variable entries found for given input.");

}

/* Display function parameters (if any) */
void display_func_paramtr()
{
	...
	printf("\n\n No function parameter entries found for given input.");
}

/* Display thread function table */
void display_thread()
{
    int i;
    if (thread_index != 0)
	...
	printf("\n\n No thread entries found for given input.");

}

/* Display log table of each function definition */
void display_log()
{
    int i;
    if (log_index != 0)
...
/* Display semaphore table (If any) */
void display_semaphr()
{
	...
	printf("\n\n No semaphore entries found for given input.");

}

/* Display mutex table (If any) */
void display_mutex()
{
    int i;
    if (mutex_index != 0)
...
/* Display function calls made by thread functions */
...
/* create_cs_log function optimizes output of critical section and prints the
   region affected by shared memory */
void create_cs_log()
{
...
/* print the critical section affected due to function calls
   made by thread functions. */
